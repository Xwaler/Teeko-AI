...
def getSurrounding(token, gameGrid):
    ...
    for shift in SURROUNDING:
        ...
        if (0 <= token[0] + shift[0] < 5 and 0 <= token[1] + shift[1] < 5 and gameGrid[token[0] + shift[0]][token[1] + shift[1]] == gameGrid[token[0]][token[1]]) or (0 <= token[0] - shift[0] < 5 and 0 <= token[1] - shift[1] < 5 and gameGrid[token[0] - shift[0]][token[1] - shift[1]] == gameGrid[token[0]][token[1]]):
...
def getAligned(player, gameGrid):
    ...
    for token in playerPos[player - 1]:
        ...
        for direction in getSurrounding(token, gameGrid):
            ...
            if not(direction == [-1, -1] and (np.abs(token[0] - token[1]) > 1) or direction == [-1, 1] and (token[0] + token[1] > 5 or token[0] + token[1] < 3)):

                i = 1

                currentLine = 1

                while 0 <= token[0] + i * direction[0] < 5 and 0 <= token[1] + i * direction[1] < 5 and gameGrid[token[0] + i * direction[0]][token[1] + i * direction[1]] == player:
...
def getAllMoves(player, gameGrid):
    ...
    for token in playerPos[player - 1]:
        ...
        for shift in SURROUNDING:
            ...
            if 0 <= token[0] + shift[0] < 5 and 0 <= token[1] + shift[1] < 5 and gameGrid[token[0] + shift[0]][token[1] + shift[1]] == 0:
